...
<body>
    <div class="header">
        ...
        <p>{{ \Carbon\Carbon::parse($start_date)->format('d M Y') }} to {{ \Carbon\Carbon::parse($end_date)->format('d M Y') }} | Generated: {{ now()->format('d M Y, h:i A') }}</p>
    </div>
    ...
    <div class="stats">
        <div class="stats-row">
            ...
            <div class="stat-item">
                ...
                @endphp
                <strong>{{ $th }}h {{ sprintf('%02d', $tm) }}m / {{ $rh }}h {{ sprintf('%02d', $rm) }}m</strong>
                <p>Actual / Required</p>
            </div>
            <div class="stat-item">
        ...
        <tbody>
            @forelse($attendances as $attendance)
                <tr>
                    ...
                    <td>
                        @php
                            $otH = floor($attendance->overtime_hours ?? 0);
                            $otM = round((($attendance->overtime_hours ?? 0) - $otH) * 60);
                        @endphp
                        {{ $otH }}h {{ sprintf('%02d', $otM) }}m
                    </td>
