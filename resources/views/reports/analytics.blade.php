...
<div class="space-y-6">
    ...
    <div class="panel">
        ...
        @if($reportData)
            @if($reportData['active_employee_count'] > 0 && $reportData['total_working_days'] > 0)
                <!-- Export Buttons -->
                        ...
                        <div class="panel bg-success-light">
                            <div class="text-center py-3">
                                <p class="text-4xl font-bold text-success">
                                    {{ $reportData['summary']['total_present'] }}/{{ $reportData['summary']['expected_slots'] }}
                                </p>
            ...
            @else
                ...
                <div class="text-center py-12">
                    <svg class="h-16 w-16 mx-auto mb-4 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                    </svg>
                    <p class="font-semibold text-lg text-gray-700 dark:text-gray-300">No Data Available</p>
                    <p class="text-sm mt-2 text-gray-600 dark:text-gray-400">
                        @if($reportData['active_employee_count'] <= 0)
                            No active employees found (excluding admins).
                        @elseif($reportData['total_working_days'] <= 0)
                            The selected date range contains no working days (Mon-Sat).
                        @else
                            No attendance records found for the selected criteria.
                        @endif
                    </p>
                    <p class="text-xs mt-1 text-gray-500">Try adjusting your filters or date range.</p>
                </div>
            @endif
        @else
            <!-- Initial State (No Generate Clicked) -->
            <div class="text-center py-12">
