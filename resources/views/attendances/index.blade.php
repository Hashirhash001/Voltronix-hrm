...
<div id="attendance-page">
    ...
    <div class="pt-5">
        ...
        <!-- Attendance Table with Inline Editing -->
                    ...
                    <tbody id="attendance-tbody">
                        @forelse($attendances as $attendance)
                            <tr data-id="{{ $attendance->id }}">
                                ...
                                <td>
                                    <input type="time"
                                           class="form-input w-32 inline-edit-time"
                                ...
                                <td><span class="font-semibold">{{ number_format($attendance->total_hours ?? 0, 2) }}h</span></td>
                                <td>
                                    <select class="form-select w-44 inline-edit-status"
                                            data-id="{{ $attendance->id }}"
                                            data-field="status">
                        ...
                        @empty
                            <tr id="no-records-row">
                                <td colspan="10" class="text-center py-12 text-white-dark">
            ...
            @if($attendances->hasPages())
                <div class="flex items-center justify-between border-t border-white-light px-6 py-4 dark:border-[#1b2e4b]" id="pagination-container">
                    ...
                    <div class="flex gap-2 items-center">
                        <button class="btn btn-sm btn-outline-secondary btn-pagination"
                                data-page="{{ $attendances->currentPage() - 1 }}"
                                {{ $attendances->currentPage() == 1 ? 'disabled' : '' }}>
                            Previous
                        </button>

                        <div class="flex gap-1" id="page-numbers">
                            @for($i = 1; $i <= $attendances->lastPage(); $i++)
                                <button class="btn btn-sm h-9 w-9 p-0 btn-pagination {{ $attendances->currentPage() == $i ? 'btn-primary bg-primary text-white font-bold' : 'btn-outline-secondary' }}"
                                        data-page="{{ $i }}">
                                    {{ $i }}
                                </button>
                            @endfor
                        ...
                        <button class="btn btn-sm btn-outline-secondary btn-pagination"
                                data-page="{{ $attendances->currentPage() + 1 }}"
                                {{ $attendances->currentPage() == $attendances->lastPage() ? 'disabled' : '' }}>
                            Next
                        </button>
                    </div>
                </div>
    ...
    <!-- Edit Modal -->
    <div id="edit-modal" class="fixed inset-0 z-[999] overflow-y-auto" style="display: none;">
        <div class="flex min-h-screen items-center justify-center px-4">
            <div class="modal-backdrop fixed inset-0 bg-black bg-opacity-60"></div>
            ...
            <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-lg shadow-xl">
                ...
                <form id="edit-attendance-form" class="p-5">
                    <input type="hidden" id="edit-attendance-id">
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                            <label class="block mb-2 text-sm font-semibold">Employee</label>
                            <input type="text" id="edit-employee-name" class="form-input bg-gray-100 cursor-not-allowed" disabled/>
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold">Date</label>
                            <input type="text" id="edit-attendance-date-display" class="form-input bg-gray-100 cursor-not-allowed" disabled/>
                        </div>

                        <div>
                            <label class="block mb-2 text-sm font-semibold">Check In <span class="text-danger">*</span></label>
                            <input type="time" id="edit-check-in" class="form-input" required/>
                        </div>
                        ...
                        <div class="md:col-span-2">
                            <label class="block mb-2 text-sm font-semibold">Notes</label>
                            <textarea id="edit-notes" rows="2" class="form-textarea"></textarea>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-2">
                        ...
                        <button type="submit" class="btn btn-success" id="btn-submit-edit">
                            <span class="submit-text">Update Attendance</span>
                            <span class="submit-loading" style="display: none;">Processing...</span>
                        </button>
                    </div>
                </form>
...
$(function () {
                ...
                if (data.attendances.length === 0) {
                    ...
                    data.attendances.forEach(function (att) {
                        ...
                        let overtimeBadge = att.overtime_hours > 0
                            ? `<span class="badge bg-warning">${parseFloat(att.overtime_hours).toFixed(2)}h</span>`
                            : '-';
                        ...
                        tbody.append(`
                            <tr data-id="${att.id}">
                                ...
                                <td>
                                    <input type="time" class="form-input w-32 inline-edit-time"
                                ...
                                <td>${parseFloat(att.regular_hours || 0).toFixed(2)}h</td>
                                <td>${overtimeBadge}</td>
                                <td><span class="font-semibold">${parseFloat(att.total_hours || 0).toFixed(2)}h</span></td>
                                <td>
                ...
                if (data.pagination.last_page > 1) {
                    let paginationHtml = `
                        <div id="pagination-container" class="flex items-center justify-between border-t border-white-light px-6 py-4 dark:border-[#1b2e4b]">
                            ...
                            <div class="flex gap-2 items-center">
                                <button class="btn btn-sm btn-outline-secondary btn-pagination" data-page="${data.pagination.current_page - 1}"
                                    ${data.pagination.current_page == 1 ? 'disabled' : ''}>Previous</button>
                                <div class="flex gap-1">`;

                    for (let i = 1; i <= data.pagination.last_page; i++) {
                        let btnClass = i == data.pagination.current_page
                    ...
                    paginationHtml += `</div>
                                <button class="btn btn-sm btn-outline-secondary btn-pagination" data-page="${data.pagination.current_page + 1}"
                                    ${data.pagination.current_page == data.pagination.last_page ? 'disabled' : ''}>Next</button>
                            </div>
                        </div>`;
    ...
    // Quick Update (inline editing) - UPDATE ONLY THE SPECIFIC ROW
    $(document).on('change', '.inline-edit-time, .inline-edit-status', function () {
        const id = $(this).data('id');
        const field = $(this).data('field');
        const $row = $(`tr[data-id="${id}"]`);

        // Store the values BEFORE making the AJAX call
        ...
        // AUTO-CHANGE STATUS TO PRESENT when both times are filled
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success && res.attendance) {
                    ...
                    // Only update the CALCULATED columns (hours)
                    $row.find('td').eq(5).text(parseFloat(att.regular_hours || 0).toFixed(2) + 'h');

                    if (att.overtime_hours > 0) {
                        $row.find('td').eq(6).html(`<span class="badge bg-warning">${parseFloat(att.overtime_hours).toFixed(2)}h</span>`);
                    } else {
                        $row.find('td').eq(6).text('-');
                    }

                    $row.find('td').eq(7).html(`<span class="font-semibold">${parseFloat(att.total_hours || 0).toFixed(2)}h</span>`);

                    // Update the status dropdown with backend value
                    $row.find('select[data-field="status"]').val(att.status);

                    // Update stats without reloading table
                    updateStatsOnly();
                }
            },
            error: function (xhr) {
