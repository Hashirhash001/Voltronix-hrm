        ...
        <div class="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-[#0e1726]">
            <div class="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                <a href="{{ route('dashboard') }}" class="main-logo flex shrink-0 items-center">
                    <img class="inline w-8 ltr:-ml-1 rtl:-mr-1" src="{{ asset('assets/images/logo/small_logo.jpg') }}" alt="image" />
                    <span class="hidden align-middle text-2xl font-semibold transition-all duration-300 ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light md:inline">HRM</span>
                </a>
            ...
            <div x-data="header" class="flex items-center space-x-1.5 ltr:ml-auto rtl:mr-auto rtl:space-x-reverse dark:text-[#d0d2d6] sm:flex-1 ltr:sm:ml-0 rtl:sm:mr-0 lg:space-x-2">
                ...
                <div class="dropdown shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="flex items-center gap-2 rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 hover:text-primary dark:bg-dark/40 dark:hover:bg-dark/60" @click="toggle">
                        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="12" cy="6" r="4" stroke="currentColor" stroke-width="1.5"/>
                            <ellipse opacity="0.5" cx="12" cy="18" rx="7" ry="4" stroke="currentColor" stroke-width="1.5"/>
                        </svg>
                    </a>
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="top-11 w-[280px] !py-0 font-semibold text-dark ltr:right-0 rtl:left-0 dark:text-white-dark dark:text-white-light/90">
                        <li>
                            <div class="flex items-center gap-3 px-4 py-4 border-b border-white-light dark:border-white-light/10">
                                <div class="flex items-center justify-center h-12 w-12 rounded-full bg-primary/10 shrink-0">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-base truncate">{{ Auth::user()->employee_name }}</h4>
                                    <p class="text-xs text-black/60 dark:text-dark-light/60 truncate">{{ Auth::user()->email }}</p>
                                </div>
                            </div>
                        </li>
                        <li>
                            <a href="{{ route('profile.edit') }}" class="flex items-center gap-2 px-4 py-3 hover:bg-white-light/10 dark:hover:bg-white-light/5" @click="toggle">
                        ...
                        <li class="border-t border-white-light dark:border-white-light/10">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex items-center gap-2 w-full px-4 py-3 text-danger hover:bg-danger/10 dark:hover:bg-danger/20" @click="toggle">
                                    <svg class="shrink-0 rotate-90" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
