        ...
        <div class="panel">
            <div class="mb-5 flex items-center justify-between">
                <h5 class="text-lg font-semibold dark:text-white-light">
                    ...
                    @if(count($expiringDocuments) > 0)
                        <span class="badge bg-danger ml-2">{{ count($expiringDocuments) }}</span>
                    @endif
                </h5>
            </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            @foreach($expiringDocuments as $doc)
                                <tr>
                                    <td>
                                        <div class="whitespace-nowrap">{{ $doc['employee']->employee_name }}</div>
                                        <div class="text-xs text-white-dark">{{ $doc['employee']->staff_number }}</div>
                                    </td>
                                    <td>{{ $doc['document_type'] }}</td>
                                    <td>{{ $doc['expiry_date']->format('d M Y') }}</td>
                                    <td>
                                        @if($doc['days_until_expiry'] < 0)
                                            <span class="font-semibold">Expired {{ abs($doc['days_until_expiry']) }} days ago</span>
                                        @else
                                    ...
                                    <td class="text-center">
                                        <a href="{{ route('employees.show', $doc['employee']->id) }}" class="text-primary hover:underline">
                                            View
                                        </a>
                                    </td>
                                </tr>
            ...
            @else
                <div class="text-center py-10">
                    <svg class="mx-auto h-20 w-20 text-success" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" fill="currentColor" opacity="0.5"/>
                        <path d="M8 12L10.5 14.5L16 9" stroke="white" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    <h3 class="mt-4 text-lg font-semibold">All Documents are Up to Date!</h3>
                    <p class="text-white-dark">No documents expiring in the next 3 months.</p>
                </div>
            @endif
        </div>

        <!-- Employees with Overtime -->
        @if(count($overtimeEmployees) > 0)
            <div class="panel mt-6">
                <div class="mb-5 flex items-center justify-between">
                    <h5 class="text-lg font-semibold dark:text-white-light">
                        Employees with Overtime This Month
                        <span class="badge bg-warning ml-2">{{ count($overtimeEmployees) }}</span>
                    </h5>
                </div>
                ...
                <div class="rounded-lg border border-white-light p-4 dark:border-[#1b2e4b]">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-success">{{ $attendanceAnalytics['present'] }}</p>
                        </div>
                ...
                <div class="rounded-lg border border-white-light p-4 dark:border-[#1b2e4b]">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-danger">{{ $attendanceAnalytics['absent'] }}</p>
                        </div>
                ...
                <div class="rounded-lg border border-white-light p-4 dark:border-[#1b2e4b]">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-warning">{{ $attendanceAnalytics['leave'] }}</p>
                        </div>
                ...
                <div class="rounded-lg border border-white-light p-4 dark:border-[#1b2e4b]">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-info">{{ number_format($attendanceAnalytics['average_hours'], 1) }}</p>
                        </div>
